# Agent setup for `portfolio`

This is a deepsec scanning workspace. Project `portfolio` was just registered
(target: `..`). Setup is incomplete — `data/portfolio/INFO.md`
...
2. **Fill in `data/portfolio/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id portfolio
pnpm deepsec process --project-id portfolio
```
